...
const SignUpFirst = () => {
  ...
  return (
    ...
    <div className="w-1/2 flex h-full">
      ...
      <section>
      <div className="text-md font-semibold text-gray-700 mb-2 mt-10">이메일</div>
        ...
        {isEmailAvailable === true && (
          <Typography color="green" className="text-sm emailsuccess-message">
            사용할 수 있는 이메일입니다.
          </Typography>
        )}
        {isEmailAvailable === false && (
          <Typography color="red" className="emailfail-message2">이미 사용된 이메일입니다.</Typography>
        )}
      </section>
      ...
      <div className="text-md font-semibold text-gray-700 mb-2 mt-4">아이디</div>
        ...
        variant="outlined"
          ...
          className="h-full whitespace-nowrap border-green-900 w-32">중복확인
        </Button>
        </div>
        {userForm.username !== "" &&
        ...
        {isUsernameAvailable === true && (
          <Typography color="green" className="text-sm idsuccess-message">사용할 수 있는 아이디입니다.</Typography>
        )}
        {isUsernameAvailable === false && (
          <Typography color="red" className="text-sm idfail-message2">이미 사용된 아이디입니다.</Typography>
        )}
      </section>

      <section>
      <div className="text-md font-semibold text-gray-700 mb-2 mt-4">비밀번호</div>
        <Input
          name="password"
          type="password"
      ...
      <div className="text-md font-semibold text-gray-700 mb-2 mt-4">비밀번호 확인</div>
      <Input
        name="verifyPassword"
        type="password"
      ...
      {userForm.verifyPassword !== "" && passwordMatch === false && (
        <Typography color="red" className="pwmatchfail-message">비밀번호와 일치하지 않습니다.</Typography>
      )}
      <div className="mb-5"></div>
      <Button className="bg-first mt-10 " onClick={() => nav("/signupfin")}>다음</Button>
      </Card>
      </div>
    </div>
  );
